...
import ContactsDisplay, { gridPageProps } from './ContactsDisplay';

describe('ContactsDisplay Component', () => {
  let wrapper;
  beforeEach(() => {
    wrapper = shallow(<ContactsDisplay />);
  });
  afterEach(() => {
    wrapper.unmount();
  });
  ...
  it('passes empty data to GridPage if still loading', () => {
    wrapper.setProps({ loading: true, contacts: ['a'] });
    expect(wrapper.find('GridPage').props().data).toEqual([]);
  });
  it('passes contacts to GridPage if not still loading', () => {
    wrapper.setProps({ loading: false, contacts: ['a'] });
